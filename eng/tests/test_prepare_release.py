...
class PrepareReleaseTests(unittest.TestCase):
    def setUp(self):
        ...
        self.manifest = {"schemaVersion": 1, "repository": "zion-sati/NetWasm",
                         ...
                         "releaseVersion": "0.2.0-rc.1", "releaseTag": "v0.2.0-rc.1",
    ...
    def run_prepare(self, version="0.2.0-rc.2"):
    ...
    def test_prepares_signed_source_tag_and_matching_manifest_without_push_or_build(self):
        ...
        self.assertEqual({"releaseVersion": "0.2.0-rc.2", "releaseTag": "v0.2.0-rc.2",
        ...
        self.assertIn(("tag", "-s", "v0.2.0-rc.2", "-m", "NetWasm 0.2.0-rc.2", "signed-source"), self.calls)
    ...
    def test_preserves_tunit_tag_prefix(self):
        self.manifest.update(repository="zion-sati/TUnit-NetWasm", releaseTag="netwasm-v0.2.0-rc.1")
        self.write_manifest()
        self.responses[("remote", "get-url", "origin")] = "git@github.com:zion-sati/TUnit-NetWasm.git"
        self.assertEqual("netwasm-v0.2.0-rc.2", self.run_prepare()["releaseTag"])
    ...
    def test_preflight_rejects_dirty_branch_private_origin_or_unapproved_identity_before_mutation(self):
        cases = [(("branch", "--show-current"), "feature"), (("status", "--porcelain=v1"), " M code"),
                 ...
                 (("tag", "--list", "v0.2.0-rc.2"), "v0.2.0-rc.2"),
                 (("ls-remote", "--tags", "origin", "refs/tags/v0.2.0-rc.2"), "existing")]
        for key, value in cases:
            with self.subTest(key=key), patch.object(release, "git", side_effect=self.git), \
                ...
                with self.assertRaises(ValueError):
                    release.prepare(self.root, "0.2.0-rc.2")
    ...
    def test_rejects_invalid_or_already_prepared_version(self):
        for version in ("not a version", "0.2.0-rc.1"):
...
class PrepareReleaseGitIntegrationTests(unittest.TestCase):
    def test_real_git_signed_commits_tag_and_version_projection(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (root / "eng/NetWasm.ReleaseVersion.txt").write_text("0.2.0-rc.1\n")
            (root / "global.json").write_text(json.dumps({"msbuild-sdks": {"NetWasm.Sdk": "0.2.0-rc.1"}}))
            ...
            with patch.object(release, "git", side_effect=local_git):
                receipt = release.prepare(root, "0.2.0-rc.2")
            self.assertEqual("", actual_git(root, "status", "--porcelain=v1"))
            self.assertEqual(receipt["sourceCommit"], actual_git(root, "rev-parse", "v0.2.0-rc.2^{commit}"))
            ...
            actual_git(root, "verify-tag", "v0.2.0-rc.2")
            self.assertEqual("0.2.0-rc.2", actual_git(root, "show", f"{receipt['sourceCommit']}:eng/NetWasm.ReleaseVersion.txt"))
