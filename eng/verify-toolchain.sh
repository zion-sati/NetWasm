...
for tool in dotnet node npm wasm-validate wasm-tools wasmtime; do
...
require_version node "$(node --version)" "v$node_version"
...
package_version() {
    node -e 'const value=require(process.argv[1]); console.log(value.packages[process.argv[2]].version)' \
