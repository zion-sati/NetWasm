...
artifact_root="${NETWASM_CANONICAL_MATRIX_ARTIFACT_DIR:-$repo_root/tests/end-to-end/component-model/artifacts/canonical-matrix}"
...
wit="$repo_root/tests/NetWasm.Compiler.Tests/Fixtures/composite-component.wit"
...
"$wit_bindgen" --wit "$wit" \
    --output "$artifact_root/CanonicalBindings.cs"
...
dotnet "$csc" -nologo -noconfig -nostdlib -langversion:latest \
    ...
    -out:"$artifact_root/CanonicalMatrixComponent.dll" \
    "$artifact_root/CanonicalBindings.cs" \
    "$repo_root/tests/end-to-end/component-model/fixtures/CanonicalMatrixComponent.cs"

entry='NetWasm.Wit.Netwasm.Test.Canonical._1._0._0.CanonicalMatrixComponent::Run'
for target in wasm32 wasm64; do
    for repeat in first second; do
        dotnet "$compiler" \
            --input "$artifact_root/CanonicalMatrixComponent.dll" \
...
result="$(cargo run --quiet --locked --release \
    ...
    --bin netwasm-wasmtime-canonical-host -- \
    "$artifact_root/component.first.wasm")"
[[ "$result" = "canonical matrix PASS" ]] || {
    echo "canonical component returned '$result'" >&2
    exit 1
}

printf 'Component Model canonical aggregate matrix acceptance PASS\n'
