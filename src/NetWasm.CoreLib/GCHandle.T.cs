// Portions derived from dotnet/runtime System.Private.CoreLib GCHandle.T.cs at commit
...
namespace System.Runtime.InteropServices
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>This type corresponds to a normal GC handle.</para>
    /// <para>
    ...
    public struct GCHandle<T> : IDisposable, IEquatable<GCHandle<T>>
        where T : class?
    {
        ...
        {
            _handle = (IntPtr)System.GCHandleRuntime.Create(target, 2);
        }
        ...
        public readonly T Target
        {
            get
            {
                ...
                CheckUninitialized(handle);
                // Skip the type check to provide lowest overhead.
                return Unsafe.As<T>(System.GCHandleRuntime.Get(handle.ToInt32())!)!;
            }
            set
            {
                IntPtr handle = _handle;
                CheckUninitialized(handle);
                System.GCHandleRuntime.Set(handle.ToInt32(), value);
            }
        }
        ...
        /// interchangeable with <see cref="GCHandle"/>.
        /// </para>
        /// </remarks>
        ...
        /// <remarks>
        /// <para>The <see cref="IntPtr"/> representation of <see cref="GCHandle{T}"/> is not
        /// interchangeable with <see cref="GCHandle"/>.</para>
        /// </remarks>
        ...
        public void Dispose()
        {
            ...
            {
                _handle = IntPtr.Zero;
                System.GCHandleRuntime.Release(handle.ToInt32());
            }
        }
        ...
        private static void CheckUninitialized(IntPtr handle)
        {
            if (handle == IntPtr.Zero)
            {
