...
namespace System.Collections.Frozen
{
    ...
    public static class FrozenDictionary
    {
            ...
            where TKey : notnull
        {
            ...
            Dictionary<TKey, TSource> dictionary = new(comparer);
            ...
            where TKey : notnull
        {
            ...
            Dictionary<TKey, TElement> dictionary = new(comparer);
            foreach (TSource item in source)
            {
            ...
            where TKey : notnull
        {
            ...
            // Optimize for keys using the default comparer. In such a case, the implementation
            ...
            if (typeof(TKey) != typeof(string) && ReferenceEquals(comparer, EqualityComparer<TKey>.Default))
            {
#if NET || NETWASM_REF_STRUCT_GENERICS
                ...
                (ReferenceEquals(comparer, EqualityComparer<TKey>.Default) || ReferenceEquals(comparer, StringComparer.Ordinal) || ReferenceEquals(comparer, StringComparer.OrdinalIgnoreCase)))
            {
                ...
                Dictionary<string, TValue> stringSource = (Dictionary<string, TValue>)(object)source;
                string[] keys = new string[source.Count];
                TValue[] values = new TValue[source.Count];
                stringSource.Keys.CopyTo(keys, 0);
                stringSource.Values.CopyTo(values, 0);
