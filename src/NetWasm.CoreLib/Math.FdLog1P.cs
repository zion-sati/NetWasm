// Managed port of Sun fdlibm s_log1p.c. Upstream: biosbits/fdlibm
...
{
    private static double FdLog1P(double x)
    {
        ...
        {           /* x < 0.41422  */
            if (ax >= 0x3ff00000)
            {       /* x <= -1.0 */
                if (x == -1.0) return -two54 / zero; /* FdLog1P(-1)=+inf */
                else return (x - x) / (x - x);  /* FdLog1P(x<-1)=NaN */
            }
            if (ax < 0x3e200000)
            {           /* |x| < 2**-29 */
        ...
        if (k != 0)
        {
            ...
            if (hu < 0x6a09e)
            {
                u = FdWithHigh(u, hu | 0x3ff00000); /* normalize u */
            }
            else
            {
                k += 1;
                u = FdWithHigh(u, hu | 0x3fe00000); /* normalize u/2 */
                hu = (0x00100000 - hu) >> 2;
            }
            f = u - 1.0;
        }
