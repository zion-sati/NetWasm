...
namespace System.Diagnostics
{
    ...
    /// They are thrown and caught so invalid input does not escape the activity operation.
    /// but the exception is suppressed, and the operation does something reasonable (typically
    /// doing nothing).
    /// </summary>
    public partial class Activity : IDisposable
    {
        internal static DateTime GetUtcNow() => DateTime.UtcNow;
...
#pragma warning restore CA1825
        ...
        /// <returns>This method does not return in the portable profile.</returns>
        /// <remarks>
        ...
        /// <returns><see langword="this" /> for convenient chaining.</returns>
        /// <remarks>
        /// The portable profile cannot represent exception type metadata. This member
        /// therefore rejects the operation before invoking listeners or changing the
        /// activity. Applications can add an explicit <see cref="ActivityEvent"/>
        /// with application-selected tags instead.
        /// </remarks>
        public Activity AddException(Exception exception, in TagList tags = default, DateTimeOffset timestamp = default)
        {
            ArgumentNullException.ThrowIfNull(exception);
            throw new PlatformNotSupportedException(SR.ActivityAddExceptionNotSupported);
        }
        ...
        {
            // Preserve the upstream non-escaping error boundary without an OS tracing hook.
            try
            {
                throw exception;
            }
        ...
        private string GenerateHierarchicalId()
        {
            ...
            if (Parent != null)
            {
                ...
                ret = AppendSuffix(Parent.Id!, (++Parent._currentChildId).ToString(), '.');
            }
            else if (ParentId != null)
            {
                ...
                ret = AppendSuffix(parentId, (++s_currentRootId).ToString("x"), '_');
            }
            else
            {
...
#pragma warning restore CA1822
        ...
        private static long GetRandomNumber() => RandomNumberGenerator.Current.Next();
