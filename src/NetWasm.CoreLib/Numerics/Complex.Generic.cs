        ...
        where T : IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
            ...
            where TChar : unmanaged, IUtfChar<TChar>
        {
            ...
            if ((typeof(TChar) == typeof(Utf8Char))
                ? !T.TryParse(MemoryMarshal.Cast<TChar, byte>(slice), componentStyle, provider, out T? real)
                : !T.TryParse(MemoryMarshal.Cast<TChar, char>(slice), componentStyle, provider, out real))
            {
                result = default;
                elementsConsumed = 0;
                return false;
            ...
            if ((typeof(TChar) == typeof(Utf8Char))
                ? !T.TryParse(MemoryMarshal.Cast<TChar, byte>(slice), componentStyle, provider, out T? imaginary)
                : !T.TryParse(MemoryMarshal.Cast<TChar, char>(slice), componentStyle, provider, out imaginary))
            {
                result = default;
                elementsConsumed = 0;
                return false;
        ...
        private static void ValidateParseStyleFloatingPoint(NumberStyles style)
        {
            ...
            if ((style & (Complex.InvalidNumberStyles | NumberStyles.AllowHexSpecifier)) != 0)
            {
                ...
                static void ThrowInvalid(NumberStyles value)
                {
                    ...
                    throw new ArgumentException("Hexadecimal style is not supported.");
                }
            }
        }
