// Managed port of Sun fdlibm e_rem_pio2.c. Upstream: biosbits/fdlibm
...
{
    private static int FdRemPio2(double x, Span<double> y)
    {
        ReadOnlySpan<int> two_over_pi = [
    ...
    0x404858EB, 0x404921FB,
        ...
        /* set z = ScaleB(|x|,ilogb(x)-23) */
        z = FdWithLow(z, FdLow(x));
        e0 = (ix >> 20) - 1046; /* e0 = ilogb(z)-23; */
        z = FdWithHigh(z, ix - (e0 << 20));
        for (i = 0; i < 2; i++)
        {
            tx[i] = (double)((int)(z));
            z = (z - tx[i]) * two24;
        ...
        n = FdKernelRemPio2(tx, y, e0, nx, 2, two_over_pi);
        if (hx < 0) { y[0] = -y[0]; y[1] = -y[1]; return -n; }
        return n;
    }
}
