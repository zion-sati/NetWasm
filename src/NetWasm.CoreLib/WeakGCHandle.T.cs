...
namespace System.Runtime.InteropServices
{
    ...
    public struct WeakGCHandle<T> : IDisposable, IEquatable<WeakGCHandle<T>>
        where T : class?
    {
        ...
        {
            _handle = (IntPtr)System.GCHandleRuntime.Create(
                target,
                trackResurrection ? 1 : 0);
        }
        ...
        public readonly bool TryGetTarget([NotNullWhen(true)] out T? target)
        {
            ...
            T? obj = Unsafe.As<T>(System.GCHandleRuntime.Get(handle.ToInt32()));
            target = obj;
            return obj != null;
        }
        ...
        public readonly void SetTarget(T target)
        {
            IntPtr handle = _handle;
            CheckUninitialized(handle);
            System.GCHandleRuntime.Set(handle.ToInt32(), target);
        }
        ...
        {
            // Keep the upstream generic-handle contract: unlike GCHandle.Free,
            // access through an uninitialized handle reports NullReferenceException.
            if (handle == IntPtr.Zero)
            {
