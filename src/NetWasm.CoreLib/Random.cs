...
namespace System
{
    ...
    public partial class Random
    {
        ...
        public virtual int Next(int maxValue)
        {
            if (maxValue < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(maxValue), maxValue, "The value must be non-negative.");
            }
        ...
        public T NextInteger<T>() where T : IBinaryInteger<T>, IMinMaxValue<T>
        {
            ...
            if (T.IsNegative(T.MaxValue))
            {
                throw new ArgumentOutOfRangeException(nameof(T), T.MaxValue, "The value must be non-negative.");
            }
        ...
        private T NextBinaryIntegerInRange<T>(T maxExclusive) where T : IBinaryInteger<T>
        {
            ...
            {
                return T.CreateTruncating(Next(Convert.ToInt32((object?)maxExclusive)));
            }
            ...
            {
                return T.CreateTruncating(NextInt64(Convert.ToInt64((object?)maxExclusive)));
            }
            ...
            {
                ulong maxExclusiveUlong = Convert.ToUInt64((object?)maxExclusive);
        ...
        public unsafe void GetItems<T>(ReadOnlySpan<T> choices, Span<T> destination)
        {
            if (choices.IsEmpty)
            {
                throw new ArgumentException("The choices span must not be empty.", nameof(choices));
            }
                ...
                choices.Length <= 256)
            {
                ...
                else
                {
                    ...
                    int mask = (int)RoundUpToPowerOf2((uint)choices.Length) - 1;
        ...
        public T[] GetItems<T>(ReadOnlySpan<T> choices, int length)
        {
            if (length < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(length), length, "The value must be non-negative.");
            }
        ...
        public string GetString(ReadOnlySpan<char> choices, int length)
        {
            ...
            GetItems(choices, characters.AsSpan());
