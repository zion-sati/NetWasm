// Managed port of Sun fdlibm s_expm1.c. Upstream: biosbits/fdlibm
...
{
    private static double FdExpm1(double x)
    {
        ...
        hx = (uint)FdHigh(x);   /* high word of x */
        ...
        else
        {
            ...
            if (k < 20)
            {
                t = FdWithHigh(t, 0x3ff00000 - (0x200000 >> k));  /* t=1-2^-k */
                y = t - (e - x);
                y = FdWithHigh(y, FdHigh(y) + ((k << 20))); /* add k to y's exponent */
            }
            else
            {
                t = FdWithHigh(t, ((0x3ff - k) << 20)); /* 2^-k */
                y = x - (e + t);
                y += one;
                y = FdWithHigh(y, FdHigh(y) + ((k << 20))); /* add k to y's exponent */
            }
