...
internal sealed class FloatingRoundIntrinsicEmitter : IRuntimeIntrinsicEmitter
{
    public void Emit(RuntimeIntrinsicEmissionRequest request, IWasmInstructionWriter code) =>
        EmitUnary(request, code, static code => code.Write(WasmInstruction.NoOperand(WasmOpcodes.F32Nearest)), static code => code.Write(WasmInstruction.NoOperand(WasmOpcodes.F64Nearest)));
