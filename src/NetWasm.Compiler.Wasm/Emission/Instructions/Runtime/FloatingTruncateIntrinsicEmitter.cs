...
internal sealed class FloatingTruncateIntrinsicEmitter : IRuntimeIntrinsicEmitter
{
    public void Emit(RuntimeIntrinsicEmissionRequest request, IWasmInstructionWriter code) =>
        EmitUnary(request, code, static code => code.Write(WasmInstruction.NoOperand(WasmOpcodes.F32Truncate)), static code => code.Write(WasmInstruction.NoOperand(WasmOpcodes.F64Truncate)));
