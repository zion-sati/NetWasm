...
internal sealed class FloatingFloorIntrinsicEmitter : IRuntimeIntrinsicEmitter
{
    public void Emit(RuntimeIntrinsicEmissionRequest request, IWasmInstructionWriter code) =>
        EmitUnary(request, code, static code => code.Write(WasmInstruction.NoOperand(WasmOpcodes.F32Floor)), static code => code.Write(WasmInstruction.NoOperand(WasmOpcodes.F64Floor)));
